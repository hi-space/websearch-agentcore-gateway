...
export function Playground({ api }: { api: PlaygroundDeps }) {
  ...
  return (
    <div className="space-y-6">
      <Card variant="panel">
        ...
        <form onSubmit={onSubmit} className="space-y-5" data-testid="playground-form">
          <label className="block">
            <span className="text-body-strong text-ink">Query</span>
            <div className="mt-2">
              <Input
                type="text"
                value={query}
          ...
          <label className="block max-w-xs">
            <span className="text-body-strong text-ink">Top-K per provider</span>
            <div className="mt-2 flex items-center gap-3">
              ...
              />
              <span className="text-body-md font-medium tabular-nums text-ink w-10 text-right">{topK}</span>
            </div>
          </label>
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {response && (
              <span className="text-caption-sm text-charcoal" data-testid="playground-latency">
      ...
      {error && (
        <Card variant="panel">
          <div className="text-body-md text-error font-medium" role="alert" data-testid="playground-error">
            Search failed: {error}
          </div>
        </Card>
      )}
      ...
      {response && (
        <Card variant="panel">
          ...
          {response.results.length === 0 ? (
            ...
            <ol className="space-y-2" data-testid="playground-results">
              {response.results.map((r, i) => (
                <li key={`${r.url}-${i}`} className="rounded-none border border-hairline bg-canvas p-6 press-collapse hover:border-hairlineStrong">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="text-caption-sm uppercase tracking-wide text-muted">#{i + 1}{r.source ? ` · ${r.source}` : ''}</div>
                      <a
                        href={r.url}
                        target="_blank"
                        rel="noreferrer"
                        className="mt-2 block text-body-strong text-ink hover:underline truncate"
                      >
                        {r.title || r.url}
                      </a>
                      <div className="text-caption-sm text-charcoal truncate">{r.url}</div>
                      {r.snippet && (
