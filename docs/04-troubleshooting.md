...
- Terraform apply 시:
  ```
  Error: Region must be us-east-1 (N. Virginia) — the AgentCore Web Search Tool connector is only available there.
  ```
- 다른 리전에서 AgentCore API 호출 시 `ResourceNotFoundException`

### 원인

AgentCore Web Search Tool 커넥터는 **us-east-1 (N. Virginia)** 에서만 제공된다. 게이트웨이 타겟은 게이트웨이와 같은 리전에만 붙을 수 있으므로 이 스택은 us-east-1로 고정되어 있다.
...
# 2. 변경 필요시
sed -i 's/aws_region = ".*/aws_region = "us-east-1"/g' \
...
aws ec2 describe-availability-zones --query 'AvailabilityZones[].RegionName' --region us-east-1

# Bedrock AgentCore 리전 가용성 확인
aws bedrockagentcore list-gateways --region us-east-1 --output table
```
...
```bash
aws sts get-caller-identity
aws cloudwatch list-metrics --namespace "AWS/Bedrock-AgentCore" --region us-east-1
```
